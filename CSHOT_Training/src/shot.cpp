...
void SHOTDescriptor::RGB2HSV (unsigned char R, unsigned char G, unsigned char B, float &H, float &S, float &V)
{
	float r = R / 255.0f;
	float g = G / 255.0f;
	float b = B / 255.0f;
...
void SHOTDescriptor::computeDesc (SHOTParams params, vtkPolyData *cloud, Feature3D *feat, double **desc, int startIndex, int endIndex){
	...
	for(int i=startIndex; i<endIndex; i++){
		...
		if (params.describeShape)
		{
			...
			//vtkDataArray*  pArray =cloud->GetPointData()->GetNormals();
			//float* normalArray =  (float*)pArray->GetVoidPointer(0);
		...
		if (params.describeColor)
		{
			...
			SHOTDescriptor::RGB2HSV (r_RGB, g_RGB, b_RGB, HRef, SRef, VRef);
			HRef /= 180.0f;
			SRef /= 255.0f;
			VRef /= 255.0f;	  //normalized HSV components
#endif
			...
			for(int j=0; j<nNeighbors; j++){	//per ogni vicino
				...
				SHOTDescriptor::RGB2HSV (r_RGB_cur, g_RGB_cur, b_RGB_cur, H, S, V);

				H /= 180.0f;
				S /= 255.0f;
				V /= 255.0f;	  //normalized HSV components

				float colorDesc = ( std::abs(HRef - H) + ((std::abs(SRef - S) + std::abs(VRef - V ))) ) / 3;
#endif
